...
const EXPANSION_KEY = 'normal.expanded';
...
// ───── Page ─────────────────────────────────────────────────────────
export default function NormalIndex(props: Props) {
    return (
        <ExpansionProvider>
            <NormalsPage {...props} />
        </ExpansionProvider>
...
function NormalsPage({ transactions, filters, options }: Props) {
    ...
    useDebouncedEffect(
        () => {
            ...
            router.get('/normal_transactions', filterForm.data, {
                preserveState: true,
                preserveScroll: true,
    ...
    const clearFilter = (key: keyof Filters) => {
        ...
        router.get('/normal_transactions', next, { preserveState: true, preserveScroll: true });
    };

    const clearAll = () => {
        filterForm.reset();
        router.get('/normal_transactions', {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const pageSummary = useMemo(() => {
        ...
        for (const t of transactions.data) {
            ...
            if (t.direction === 'credit') credit += amt;
            else debit += amt;
        }
        return { count: transactions.data.length, credit, debit, net: credit - debit };
    }, [transactions.data]);
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Normal Transactions" />

            <PageHeader
                title="Normal Transactions"
                description="Your settled financial activity"
                actions={
                    <>
                        ...
                        <PrimaryButton onClick={() => setFormOpen({ mode: 'create' })}>
                            <PlusIcon className="mr-1.5 h-4 w-4" /> Add Transaction
                        </PrimaryButton>
                    </>
                }
            />
                    ...
                    <span className="flex flex-wrap items-center gap-3">
                        <span>
                            ...
                            <span className="font-semibold tabular-nums text-success">
                                +E£ {fmt(pageSummary.credit)}
                            </span>
                        </span>
                        <span>
                            Debit:{' '}
                            <span className="font-semibold tabular-nums text-destructive">
                                -E£ {fmt(pageSummary.debit)}
                            </span>
                        </span>
                        <span>
                            Net:{' '}
                ...
                {transactions.data.length === 0 ? (
                    hasActiveFilters ? (
                        ...
                        <EmptyState
                            ...
                            description="Record income or expenses as they happen — or stage them as drafts first."
                            cta={
                                <PrimaryButton onClick={() => setFormOpen({ mode: 'create' })}>
                                    <PlusIcon className="mr-1.5 h-4 w-4" /> Add your first transaction
                                </PrimaryButton>
                            }
                        />
                    )
            ...
            <TransactionFormModal
                ...
                endpoint={formOpen?.mode === 'edit' ? `/normal_transactions/${formOpen.row.id}` : '/normal_transactions'}
                categories={options.categories}
                wallets={options.wallets}
                monthYears={options.month_years}
                title={formOpen?.mode === 'edit' ? 'Edit transaction' : 'New transaction'}
            />
                ...
                <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                            {row
                                ? `"${row.name}" will be unfinalized and moved to your Drafts list.`
                                : ''}
                        </p>
                    </div>
                </div>
        ...
        <div className="group rounded-xl border border-border bg-card shadow-sm transition-colors hover:border-border/80">
            <div className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex gap-1 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100 pointer-coarse:opacity-100">
                    <IconButton onClick={onMoveToDraft} title="Move to drafts" tone="warning">
