        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px;">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title fw-bold text-secondary" id="staticBackdropLabel"
                    ...
                    <i class="bi bi-file-earmark me-2 text-warning"></i> Transfer to Draft
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form method="POST" action="/normal_transactions/transfer_to_draft">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select select2-inside-modal"
                                style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 0.5rem 1rem;"
                                id="wallet_id{{ $transaction->id }}" name="wallet_id" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select"
                                style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 0.5rem 1rem;"
                                id="monthYearSelect{{ $transaction->id }}" name="month_year_id" required>
                ...
                <div class="modal-footer border-0 pt-0">
                    ...
                    <button type="submit" class="btn btn-warning rounded-pill px-4"><i
                            class="bi bi-file-earmark me-1"></i> Transfer</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        form.addEventListener("submit", function () {
            ...
            submitButton.innerHTML = `<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Transferring...`;
        });
        const modalElement = document.querySelector(modalId);
        modalElement.addEventListener("shown.bs.modal", function () {
            $('#categorySelect{{ $transaction->id }}').select2({ dropdownParent: $(modalId), placeholder: "Select a category", width: '100%' });
            $('#wallet_id{{ $transaction->id }}').select2({ dropdownParent: $(modalId), placeholder: "Select a wallet", width: '100%' });
            $('#monthYearSelect{{ $transaction->id }}').select2({ dropdownParent: $(modalId), placeholder: "Select Month-Year", width: '100%' });
        });
    });
</script>